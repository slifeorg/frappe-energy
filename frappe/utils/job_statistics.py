...
def get_job_statistics(user, roles):
    ...
    sql_query = f"""
        SELECT
            ...
            CAST(SUM(CASE WHEN hers_status NOT IN ({status_placeholders}) (type = 'HERS & PERMITS' OR type = 'HERS TESTS ONLY') THEN 1 ELSE 0 END) AS UNSIGNED) AS hers_test_active_job,
            CAST(SUM(CASE WHEN permit_status NOT IN ({status_placeholders}) type = 'PERMIT ONLY'  THEN 1 ELSE 0 END) AS UNSIGNED) AS permit_active_job,
